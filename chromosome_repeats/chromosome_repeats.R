...
setwd("/Users/mschatz/Dropbox/Documents/Projects/T2T/chromosome_repeats")
...
#ggplot(chm_long, aes(x=chr, y=len)) + geom_col(aes(fill=type))
...
#ggplot(hg38_long, aes(x=chr, y=len)) + geom_col(aes(fill=type))

all_long <- rbind(hg38_long, chm_long)
#ggplot(all_long, aes(x=chr, y=len, fill=sample)) + geom_col(aes(fill=type))
